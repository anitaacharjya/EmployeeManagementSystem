					...
					<table class="min-w-full">
						...
						<tbody class="text-gray-700 text-sm font-light">
							...
							<tr class="table-row border-b border-gray-200 hover:bg-blue-100">
								<td class="table-cell">
									<a href="ShiftTimeOfUser.jsp?emp_id=<%=user.getEmp_id()%>" class="relative emp-link" data-emp-id="<%=user.getEmp_id()%>">
										#<%=user.getEmp_id()%>
										<% if (i == 0) { %>
										<span class="popup">Tap here to see the attendance list</span>
										<% } %>
									</a>
								</td>
